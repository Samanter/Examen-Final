...
void moveBall(char pos) {
	if (pos == 'w') zpos -= 0.01f;
	else if (pos == 's') zpos += 0.01f;
	else if (pos == 'a') xpos -= 0.01f;
	else if (pos == 'd') xpos += 0.01f;
}
...
i32 main() {
	...
	Model* planet = new Model(files, "ball/ball.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		model = glm::scale(model, glm::vec3(expand, expand, expand));
